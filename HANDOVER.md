...
- **User-model document:** after every completed turn the client fires a
  background call to `/api/profile`, which updates a markdown document
  ...
  verbatim quotes) by merging the prior document with the current transcript.
  The client sends it with the next chat request, where it's injected as the
  volatile system block.
- **Memory across conversations:** the document persists in localStorage
  (key `talk-to-yourself:user-model`) and is loaded on mount, so a fresh chat
  starts already knowing the user. The merge semantics in `PROFILE_CORE`
  (preserve prior knowledge unless contradicted) are what make this safe —
  without them a new short conversation would wipe the accumulated model.
- **Two-column UI:** chat on the left, the rendered user-model document
  (react-markdown, styled in `globals.css` under `.model-body`) in a panel on
  the right, with "save .md" (download) and "forget me" (erase memory)
  buttons. Columns stack on narrow screens.
...
| `app/api/profile/route.ts` | POST `{messages, profile?}` → JSON `{profile}`: merges the prior user-model document with the current transcript (non-streaming, called in background). |
| `app/page.tsx` | Client chat UI: streaming bubbles, background profile refresh, localStorage persistence, right-hand model panel with markdown rendering + save/forget. |
...
8. **No mimicry stages (user's explicit decision, 2026-06):** full mimicry
   ...
   user model lives in the AI-maintained document instead, updated after
   every turn by merging the prior document with the transcript (cheap
   thanks to caching).
...
- Conversation state is client-side only (full history POSTed each turn) and
  does not survive a reload — only the user-model document does (localStorage).
